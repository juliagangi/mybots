...
class SOLUTION:
    ...
    def Create_Body(self,parentID,parentOrChild):
        '''
        if id >= c.numberOfGenerations:
            id = self.myID
            while id >= c.numberOfGenerations:
                id = id - c.numberOfGenerations
            pyrosim.Start_URDF("body" + str(id) + ".urdf")
        '''
        if parentOrChild == 'child': # don't want to create new file w random 
            pyrosim.Start_URDF("body" + str(self.myID) + ".urdf")
            #childFile = URDF.load("body" + str(self.myID) + ".urdf")
            parentFile = URDF.load("body" + str(parentID) + ".urdf")
            parentFile.save("body" + str(self.myID) + ".urdf")
        else:
            pyrosim.Start_URDF("body" + str(self.myID) + ".urdf")
            overalllink = 0
